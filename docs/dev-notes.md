...
## 10. Service Worker 把前端改动挡住了（cache-first 的代价）

`sw.js` 原来对 `/static/` 用 **cache-first**，注释里给的理由是"它们有版本号
或很少变"。**这个前提不成立**：项目没有构建步骤，就没有文件名指纹；
`VERSION` 是手写常量，没人会记得改。

后果不是"样式可能有点旧"，而是**前端改动永远到不了已经装过 SW 的浏览器**。
实测踩到，而且极具误导性：新写的备注组件样式一直没生效，看到的是
浏览器默认渲染 —— 原生 disclosure 三角没被隐藏（像个文字字形 ▶）、
字号跟着正文走（显得比标题还大）、折叠与展开的文案**同时显示**。
看起来完全是"CSS 写错了"，实际是那份 CSS 压根没被加载。

排查方式：打开 DevTools → Network，看 `app.css` 是不是 `(ServiceWorker)`；
或者 Application → Service Workers 里 Unregister 一下立刻恢复正常。

**现在的策略：`/static/` 也走 network-first**，断网才退回缓存。代价是每个静态
资源多一次条件请求（有 ETag / 304 兜着，通常是空响应），换来"改了就能看到"。
对一个个人应用来说这个交换明显划算。

`VERSION` 仍然保留，但它不再承担"让改动生效"的责任，只负责别让废弃缓存
永远堆着。

**给未来的自己：改完前端先确认 SW 已经更新。** 新 `sw.js` 会在下一次导航时被
抓取（`/sw.js` 本身带 `Cache-Control: no-cache`），`skipWaiting` +
`clients.claim` 之后生效——但**当前这个已加载的页面**仍然用着旧资源，
所以需要刷新一次。`tests/test_pwa.py` 里有用例钉住这条策略，
防止有人以"省流量"为由改回 cache-first。

---

## 11. 又一个"测试全绿但功能坏了"：网页审核吞掉备注

`draft.js` 的 `collect()` 只回传了 4 个字段：

```js
{ title, category, due_at, priority }      // notes / source_quote 不见了
```

而 `draft.html` 也**不渲染** notes。于是"在网页上二次审核"这条路上，模型
抽出来的备注被静默清空；而接口直接确认（不带 `items`，用草稿里现存的）
反而保留 —— 表现得很像"偶发"，实际是两条路径行为不一致。

为什么没被测出来：既有的确认用例都是直接构造一个理想的 `items` 回传，
**天然带着 notes**，所以怎么测都是过的。用例绕过了真实客户端的行为。

**对策：回归用例必须模拟客户端的真实行为，而不是它的理想行为。**
`test_web_review_does_not_lose_notes` 先用正则从确认页 HTML 里读回
`data-field` 控件的值、再拼成 payload —— 字段一旦不在页面上，用例直接失败。
另有一条源码级守卫钉住 `collect()` 确实带上了这两个字段
（前者证明"页面给了"，后者证明"客户端会发"，两边都钉住才算闭环）。
